...
const inviteUrl = "https://guilherme-henrique-direito.up.railway.app/";
const hasFullName = (value) => value.trim().split(/\s+/).filter(Boolean).length >= 2;
const hasValidPhone = (value) => { const digits = value.replace(/\D/g, ""); return digits.length === 10 || digits.length === 11; };
...
function GuestView({ onRespond }) {
  ...
  const companionReady = form.acomp === 0 || (hasFullName(form.acompanhanteNome) && form.acompanhanteSexo && (form.acompanhanteSexo === "homem" || form.acompanhanteSandalia));
  const ready = hasFullName(form.nome) && hasValidPhone(form.fone) && form.sexo && (form.sexo === "homem" || form.sandalia) && companionReady;
  ...
  const hint = !form.nome.trim() ? "Preencha seu nome completo." : !hasFullName(form.nome) ? "O nome completo precisa ter pelo menos duas palavras." : !form.fone.trim() ? "Informe seu WhatsApp com DDD." : !hasValidPhone(form.fone) ? "Digite um WhatsApp válido com 10 ou 11 números." : !form.sexo ? "Escolha uma das duas opções acima." : form.sexo === "mulher" && !form.sandalia ? "Falta escolher o número da sua sandália." : form.acomp === 1 && !form.acompanhanteNome.trim() ? "Informe o nome completo do acompanhante." : form.acomp === 1 && !hasFullName(form.acompanhanteNome) ? "O nome do acompanhante precisa ter pelo menos duas palavras." : form.acomp === 1 && !form.acompanhanteSexo ? "Informe se o acompanhante é homem ou mulher." : form.acompanhanteSexo === "mulher" && !form.acompanhanteSandalia ? "Falta escolher o número da sandália da acompanhante." : "Você poderá corrigir sua resposta até 1 de outubro.";
  ...
  return <main className="narrow invite">
    ...
    <section className="form-section">
      ...
      <div className="fields">
        <Field label="Nome completo" required minLength="3" value={form.nome} onChange={(e) => update("nome", e.target.value)} placeholder="Nome e sobrenome" />
        <Field label="WhatsApp" required type="tel" inputMode="numeric" value={form.fone} onChange={(e) => update("fone", e.target.value)} placeholder="(83) 90000-0000" hint="Obrigatório. Digite o DDD e o número." />
      </div>
...
function Login({ unlock, back }) {
  ...
  const submit = () => password.trim().toLowerCase() === "pokozinmeufi" ? unlock() : setError(true);
  return <main className="narrow login"><div className="accent-line"/><p className="eyebrow">Área restrita</p><h1>Painel do formando</h1><p>Os dados dos convidados são visíveis apenas para a formanda. Informe a senha de acesso para continuar.</p>
    ...
    {error && <small className="error">Senha incorreta. Tente novamente.</small>}
...
function GuestEditor({ guest, onClose, onSave }) {
  const [draft,setDraft]=useState({...guest}), [error,setError]=useState(""), [saving,setSaving]=useState(false);
  const set=(key,value)=>setDraft((current)=>({...current,[key]:value}));
  const save=async()=>{setSaving(true);setError("");try{await onSave(draft);onClose()}catch(reason){setError(reason.message)}finally{setSaving(false)}};
  return <div className="modal-backdrop" onMouseDown={(event)=>event.target===event.currentTarget&&onClose()}><section className="modal guest-editor" role="dialog" aria-modal="true" aria-labelledby="guest-title"><button className="modal-close" aria-label="Fechar" onClick={onClose}>×</button><p className="eyebrow red">Lista de convidados</p><h2 id="guest-title">Editar convidado</h2><div className="editor-grid"><Field label="Nome completo" value={draft.nome} onChange={(e)=>set("nome",e.target.value)}/><Field label="WhatsApp" value={draft.fone} onChange={(e)=>set("fone",e.target.value)}/><label className="field"><span>Resposta</span><select value={draft.status} onChange={(e)=>set("status",e.target.value)}><option value="vou">Vou</option><option value="nao">Não vou</option></select></label><label className="field"><span>Convidado</span><select value={draft.sexo} onChange={(e)=>set("sexo",e.target.value)}><option value="homem">Homem</option><option value="mulher">Mulher</option></select></label>{draft.sexo==="mulher"&&<Field label="Sandália do convidado" type="number" min="33" max="40" value={draft.sandalia||""} onChange={(e)=>set("sandalia",e.target.value)}/>}<label className="field"><span>Levará acompanhante?</span><select value={draft.acomp?"1":"0"} onChange={(e)=>set("acomp",Number(e.target.value))}><option value="0">Não</option><option value="1">Sim</option></select></label>{draft.acomp===1&&<><Field label="Nome do acompanhante" value={draft.acompanhanteNome||""} onChange={(e)=>set("acompanhanteNome",e.target.value)}/><label className="field"><span>Acompanhante</span><select value={draft.acompanhanteSexo||""} onChange={(e)=>set("acompanhanteSexo",e.target.value)}><option value="">Selecione</option><option value="homem">Homem</option><option value="mulher">Mulher</option></select></label>{draft.acompanhanteSexo==="mulher"&&<Field label="Sandália da acompanhante" type="number" min="33" max="40" value={draft.acompanhanteSandalia||""} onChange={(e)=>set("acompanhanteSandalia",e.target.value)}/>}</>}</div>{error&&<small className="error modal-message">{error}</small>}<div className="modal-actions"><button className="button secondary" onClick={onClose}>Cancelar</button><button className="button primary" disabled={saving} onClick={save}>{saving?"Salvando…":"Salvar alterações"}<b>→</b></button></div></section></div>;
}

function Dashboard({ logout, responses, setResponses }) {
  ...
  const rows = useMemo(() => {
    const base = allGuests.map(g => ({name:g[0], status:g[1], shoe:[g[3] !== "—" ? g[3] : null,g[7] && g[7] !== "—" ? `Acomp. ${g[7]}` : null].filter(Boolean).join(" · ") || "—", companion:g[1] === "vou" && g[4] ? g[8] ? `${g[8]} (${g[9]})` : `+${g[4]}` : "—", phone:g[5], date:g[6],source:g[10]}));
  ...
  const saveGuest=async(draft)=>{const request=await fetch("/api/responses",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({...draft,originalIdentity:editingGuest.identity})});const saved=await request.json();if(!request.ok)throw new Error(saved.error||"Não foi possível editar.");setResponses((current)=>current.map((item)=>item.identity===editingGuest.identity?saved:item))};
  const removeGuest=async()=>{setRemoveError("");const request=await fetch("/api/responses",{method:"DELETE",headers:{"Content-Type":"application/json"},body:JSON.stringify({identity:removingGuest.identity})});const data=await request.json();if(!request.ok){setRemoveError(data.error||"Não foi possível remover.");return}setResponses((current)=>current.filter((item)=>item.identity!==removingGuest.identity));setRemovingGuest(null)};
  useEffect(() => { const close = (event) => { if (event.key === "Escape") { setEditingTotal(false); setEditingMessage(false); setEditingGuest(null); setRemovingGuest(null); } }; window.addEventListener("keydown", close); return () => window.removeEventListener("keydown", close); }, []);
  ...
  return <main className="dashboard"><section className="dash-head"><div><p className="eyebrow red">Painel do formando</p><h1>Quem já respondeu</h1></div><div className="dash-tools"><div className="counter-wrap"><span>Convites a enviar</span><button className="edit-total" onClick={openTotalEditor}><b>{effectiveTotal}</b><span>Editar número de convidados</span></button></div><p>{allGuests.length} de {effectiveTotal} convites respondidos · prazo 1 out</p><button className="small primary" onClick={()=>setShowLink(!showLink)}>Link do convite</button><button className="small outline" onClick={doExport}>{exported ? "Lista exportada ✓":"Exportar lista"}</button><button className="small outline muted" onClick={logout}>Sair</button></div></section>
    ...
    <section className="charts"><div><p className="eyebrow">Tamanhos pedidos</p>{Object.entries(sizes).sort().map(([size,n])=><div className="bar" key={size}><b>{size}</b><i><em style={{width:`${n/max*100}%`}}/></i><strong>{n}</strong></div>)}</div><div><p className="eyebrow">Composição dos confirmados</p><div className="composition"><p><b>Mulheres</b><b>{women}</b></p><i><em className="redbar" style={{width:`${attending.length ? women/attending.length*100 : 0}%`}}/></i><p><b>Homens</b><b>{men}</b></p><i><em style={{width:`${attending.length ? men/attending.length*100 : 0}%`}}/></i></div><p className="chart-note">Taxa de resposta de {pct}%. Quem ainda não respondeu recebe um lembrete três dias antes do prazo.</p></div></section>
    <section className="guest-list"><div className="list-head"><p className="eyebrow">Lista de convidados</p><div><div className="search"><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Pesquisar nome ou telefone"/>{search&&<button onClick={()=>setSearch("")}>Limpar</button>}</div><div className="filters">{["Todos","Vou","Não vou","Aguardando"].map(x=><button className={filter===x?"selected":""} onClick={()=>setFilter(x)} key={x}>{x}</button>)}</div></div></div>
      <div className="table-wrap"><table><thead><tr><th>Convidado</th><th>Resposta</th><th>Acomp.</th><th>Sandália</th><th>WhatsApp</th><th>Respondeu</th><th>Ações</th></tr></thead><tbody>{rows.map((r,i)=><tr key={i}><td>{r.name}</td><td><span className={`tag ${r.status}`}>{r.status==="vou"?"Vou":r.status==="nao"?"Não vou":"Aguardando"}</span></td><td>{r.companion}</td><td>{r.shoe}</td><td>{r.phone}</td><td>{r.date}</td><td>{r.source?<div className="row-actions"><button onClick={()=>setEditingGuest(r.source)}>Editar</button><button className="danger" onClick={()=>{setRemoveError("");setRemovingGuest(r.source)}}>Remover</button></div>:r.status==="pendente"?"—":<span className="sample-label">Exemplo</span>}</td></tr>)}</tbody></table>{!rows.length&&<p className="empty">Nenhum convidado encontrado para esta busca.</p>}</div>
    </section>
    ...
    {editingGuest&&<GuestEditor guest={editingGuest} onClose={()=>setEditingGuest(null)} onSave={saveGuest}/>} 
    {removingGuest&&<div className="modal-backdrop" onMouseDown={(event)=>event.target===event.currentTarget&&setRemovingGuest(null)}><section className="modal confirm-modal" role="alertdialog" aria-modal="true" aria-labelledby="remove-title"><button className="modal-close" aria-label="Fechar" onClick={()=>setRemovingGuest(null)}>×</button><p className="eyebrow red">Ação permanente</p><h2 id="remove-title">Remover convidado?</h2><p>A resposta de <strong>{removingGuest.nome}</strong> será removida definitivamente da lista.</p>{removeError&&<small className="error modal-message">{removeError}</small>}<div className="modal-actions"><button className="button secondary" onClick={()=>setRemovingGuest(null)}>Cancelar</button><button className="button danger-button" onClick={removeGuest}>Remover convidado</button></div></section></div>}
  </main>;
}
...
export default function Home() {
  ...
  return <><Header view={view} setView={setView}/>{view === "guest" ? <GuestView onRespond={registerResponse}/> : unlocked ? <Dashboard responses={responses} setResponses={setResponses} logout={()=>{setUnlocked(false);setView("guest")}}/> : <Login unlock={()=>setUnlocked(true)} back={()=>setView("guest")}/>}</>;
}
